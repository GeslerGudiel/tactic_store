...
function enviarCorreoConfirmacion($correo) {
    ...
    $mensaje = "
        ...
        <p>Te informamos que tu contraseña ha sido actualizada correctamente. Si no has realizado este cambio, por favor contacta con soporte lo antes posible.</p>
    ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($nueva_contrasena !== $confirmar_contrasena) {
        ...
        try {
            ...
            $query = "SELECT id_cliente, correo, fecha_token FROM cliente WHERE token_recuperacion = :token";
            ...
            if ($cliente) {
                $fecha_token = new DateTime($cliente['fecha_token']);
                $ahora = new DateTime();
                $intervalo = $fecha_token->diff($ahora);

                if ($intervalo->i <= 30) {
                    // Actualizar la contraseña y eliminar el token
                    $query = "UPDATE cliente SET contrasena = :contrasena, token_recuperacion = NULL, fecha_token = NULL WHERE id_cliente = :id_cliente";
                    $stmt = $db->prepare($query);
                    ...
                    enviarCorreoConfirmacion($cliente['correo']);

                    redirectWithMessage('login_cliente.php', 'success', 'Contraseña actualizada correctamente. Ahora puedes iniciar sesión.');
                } else {
